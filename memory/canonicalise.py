...
Canonicalisation rules for v1:
- lowercase, collapse whitespace
- strip timestamps (ISO-8601, [HH:MM:SS], bare HH:MM:SS)
- redact API-key-shaped strings BEFORE anything is keyed or stored
- absolute paths become repo-relative when a repo root can recover them;
  otherwise the documented fallback is basename-only (two different files
  sharing a basename collapse to one key — conservative, mirrors v1 scope)
- error family kept (`KeyError`, `AssertionError`, `ModuleNotFoundError`,
  …, lowercased); pytest node ids win over quoted symbols

failure_key format: `{tool}|{error_family}|{symbol_or_test_id}|{relpath}`.
Missing fields degrade to a conservative key — these functions never raise
in the hook path.
"""
import hashlib
import json
