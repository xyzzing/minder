...
DEFAULT_STATE_DIR = "~/.local/state/minder"
...
def _auth_headers():
    """Authorization header for the sink's write path, from
    `STATE_DIR/sink.token` (mtime-cached; the sink may recreate it between
    hook spawns). Empty dict when the file is absent or unreadable — the
    server then decides (no token in force → open)."""
    path = os.path.expanduser(os.path.join(
        os.environ.get("MINDER_STATE_DIR", DEFAULT_STATE_DIR), TOKEN_NAME))
    try:
        key = (path, os.path.getmtime(path))
    except OSError:
        return {}
    if _TOKEN["key"] == key:
        return dict(_TOKEN["headers"])
    try:
...
def _post(path, payload, timeout_ms=None, base=None):
    ...
    try:
        req = urllib.request.Request(
            ...
            headers={**_CT, **_auth_headers()}, method="POST")
